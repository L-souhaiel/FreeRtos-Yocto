...
void printValueOnConsole()
{
    int ValueReceived = 0;

    while(1)
    {
        xQueueReceive(xQueue,&ValueReceived,portMAX_DELAY);
...

{
    ...
    xTaskCreate(led_task,"Task Led",256,NULL,1,NULL);
    xTaskCreate(printValueOnConsole,"Task Console",256,NULL,1,NULL);
    vTaskStartScheduler();
    while(1){};
